...
históricamente `RESOLVED`. Por decisión humana del 2026-08-19, DB-001 está `READY` y es la única Work
Package autorizada para implementación. Todas las demás Work Packages que no están ya finalizadas
permanecen `DRAFT` y no están autorizadas.
...
| Autorizada | DB-001 | `READY`; autorización humana del 2026-08-19 tras verificar FND-003/FND-005 `DONE`, scope Platform-only y ausencia de bloqueos P0 aplicables |
...
`BLK-FND-005-001` está `RESOLVED`; DB-001 está `READY` por autorización humana del 2026-08-19 y es
la única WP autorizada. Todas las demás WPs aún no finalizadas permanecen `DRAFT` y no están
autorizadas.
...
1. Conservar BAS-001/Gate 0 y FND-001, FND-002, FND-003, FND-004 y FND-005 como `DONE`; las cinco WPs
   FND finalizadas quedan respaldadas por sus PR y reportes de implementación respectivos.
...
`test:golden` no puede simular aprobación. En Gate 3 ejecutará un control real de aplicabilidad: debe reportar `NOT_APPLICABLE_SCOPE` porque no existe cálculo/política contable en el Walking Skeleton y debe fallar si detecta una superficie contable publicada sin dataset aprobado. No se contabiliza como PASS contable. FND-005 activa exclusivamente el proyecto real `test:integration -- --project local-infrastructure`; los proyectos de integración de WPs futuras conservan estados explícitos de scope y no se presentan como implementados. Los demás controles aplicables deben ejecutar pruebas reales.
...
El cierre documental de FND-004 queda respaldado por el PR #11, el merge commit real
`7a9e35aac22fff1b6c0ad36d5547875d4c45f9de` y
`docs/plans/reports/FND-004-report.md`. La evidencia humana post-merge aprobada registra `npm ci`
exit 0, `npm run verify` exit 0, unit 26/26, architecture 35/35, golden
`NOT_APPLICABLE_SCOPE` bajo ownership de QA-002 mediante inspección real, build exit 0,
backend-smoke de FND-003 exit 0, `git diff --check` exit 0 y working tree limpio. El checkout limpio
previo validó architecture 35/35 y verify exit 0. `packages/design-system` no fue modificado,
`testkit` se consume mediante `@trazactivo/testkit` y domain/policy-engine conservan pureza
deny-by-default. `BLK-FND-004-001` permanece históricamente `RESOLVED`. Este cierre no autorizó por
sí mismo FND-005 ni ninguna WP adicional; la autorización posterior y separada de FND-005 se registra
en su Work Package y en el estado vigente de este plan.

El cierre documental de FND-005 queda respaldado por el PR #14, el merge commit real
`4084e3a436ef62636eb4ec523f95dd6c39967696` y
`docs/plans/reports/FND-005-report.md`. El head implementado fue
`21a90e430a73d1ed9855720ea3da415ca54f1ebb` y el clean checkout validó el commit candidato
`ea0e14cee984d7cf2bc672d21a0233449c91803b`. La evidencia post-merge aprobada registra SQL Server
real con Platform/A/B, aislamiento e independencia A/B, integration `local-infrastructure` real,
unit 26/26, architecture 50/50, a11y 5/5, build y backend-smoke PASS, `npm run verify` exit 0,
`git diff --check` exit 0 y working tree limpio. `BLK-FND-005-001` conserva como `RESOLVED` la
historia del Engine indisponible, el `Msg 911`, la separación del bootstrap y sus regresiones. Este
cierre no autoriza ninguna WP adicional.

La autorización posterior de DB-001 se registra mediante decisión humana del 2026-08-19 y queda
acotada a la foundation Prisma de Platform DB. FND-003 y FND-005 están `DONE`; DEC-DATA-001 fija
Prisma y DEC-CLI-003 fija Catalog/Resolver server-side antes del futuro Prisma Client DB. DB-001 no
implementa modelos funcionales de catálogo o identidad, migrations, seeds ni acceso Client DB.
DB-002, DB-003, API-001, AST-001, las WPs CLI y todas las demás WPs no finalizadas permanecen
`DRAFT` y no autorizadas.
...
- FND-001, FND-002, FND-003, FND-004 y FND-005 están `DONE`; `BLK-FND-004-001` y
  `BLK-FND-005-001` permanecen históricamente `RESOLVED`. DB-001 es la única WP en `READY` por
  autorización humana del 2026-08-19; ninguna WP adicional está autorizada o en ejecución.
...
FND-002, FND-003, FND-004 y FND-005 están `DONE`; `BLK-FND-005-001` está `RESOLVED`. No existe
ninguna otra WP en `READY`: DB-001 es la única WP `READY` y autorizada por decisión humana del
2026-08-19. Todas las demás WPs aún no finalizadas permanecen `DRAFT` y no están autorizadas. El
Walking Skeleton sólo se considera implementado y aprobado localmente cuando:
